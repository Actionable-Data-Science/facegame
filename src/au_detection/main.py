...
os.chdir("src/au_detection")

# print(sys.argv[1])

splitted_path = sys.argv[1].split("/")

sleep(1) # replace by checking if file exists and continuing when it does

image = cv2.imread(f"../../participant-images/{splitted_path[-1]}")

predictor = dlib.shape_predictor("shape_predictor_68_face_landmarks.dat")
pca_model = joblib.load("hog_pca_all_emotio.joblib")
classifier = joblib.load("svm_568.joblib")
scaler = joblib.load("hog_scalar_aus.joblib")
...
for i in range(len(offline_pred_aus)):
    if offline_pred_aus[i] ==1:
        predictions_output.append(str(au_array[i]))


print(', '.join(predictions_output))
