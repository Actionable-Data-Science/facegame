...
function getFalseAUs(activeAUs, targetAUs) {
  ...
  return {"falsePositive": falsePositiveAUs, "falseNegative": falseNegativeAUs};
}
...
function fee(activeAUs, targetAUs){
  ...
  if (typeof targetAUs == "undefined"){
    ...
    const todo = faceExpressionToNaturalLanguage(falseAUs["falseNegative"], ask_for='prescription');
    const notTodo = faceExpressionToNaturalLanguage(falseAUs["falsePositive"], ask_for='prescription_negative');
    retVal = todo + notTodo;
  }
  if (retVal.length > 1){
...
function faceExpressionToNaturalLanguage(listAu, askFor = 'description') {
  ...
  if (!(requests.includes(askFor))) {
    ...
    const chinNoseInfo = feeDictionary["chin_nose"][lowerFaceUpdownChinnoseCode][askFor];
